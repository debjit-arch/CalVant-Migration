...
const canDownloadPolicies = (roles = []) =>
  roles.some((r) =>
    ["root", "super_admin", "auditor", "audit_manager"].includes(r),
  );
...
const TrustCentreContent = () => {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const msg =
        e.status === 404
          ? "Trust Centre not found for this organisation."
          : e.status === 403
            ? "You don't have permission to view the Trust Centre."
            : "Trust Centre is not available yet.";
      setError(msg);
      console.error("Trust Centre fetch failed:", e.status, e.message);
    } finally {
      setLoading(false);
    }
  }, [token, router]);
  ...
  if (error || !tc) {
    return (
      ...
      >
        ...
        <h2
          style={{ margin: 0, color: "#1e293b", fontWeight: 700, fontSize: 20 }}
        >
        ...
        >
          Your organisation's Trust Centre hasn't been published yet. Contact
  ...
  return (
    ...
    >
        ...
        {tc.policies?.length > 0 && (
          ...
          >
                  ...
                  {canDownload ? (
                    <motion.button
                      ...
                      style={{
                        ...
                        borderRadius: 8,
                        background:
                          downloading === p.name ? "#e2e8f0" : "#8b5cf6",
                        color: downloading === p.name ? "#94a3b8" : "white",
                        border: "none",
                        cursor: downloading === p.name ? "default" : "pointer",
                        fontWeight: 600,
                        fontSize: 12,
                        flexShrink: 0,
                      }}
            ...
            {!canDownload && (
              ...
              >
                <Lock size={12} />
                Policy downloads are available to auditors and administrators
                only.
              </div>
            )}
          </SectionCard>
      ...
      >
        <p
          style={{ margin: 0, color: "#94a3b8", fontSize: 12, fontWeight: 500 }}
        >
          &copy; {new Date().getFullYear()} CalVant - Trust Centre - Read-only
          view
        </p>
      </footer>
...
export default TrustCentreContent;

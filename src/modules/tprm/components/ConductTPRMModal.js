...
//       // ✅ Only show users who have "tprm" in their modules array
...
//     allQuestionIds.every((id) => selectedQIds.includes(id));

//   const toggleSelectAll = () => {
//     setSelectedQIds(allSelected ? [] : [...allQuestionIds]);
//   };
...
//       }
//       // Immediately send to vendor — no draft step
//       if (questionnaireId) {
...
//       setError(
//         e.message ||
//           (editingId ? "Failed to submit" : "Failed to create and submit"),
//       );
//     } finally {
//       setSaving(false);
//     }
...
//               {/* Sorting Tabs - Now correctly placed inside the List view */}
...
//                 /* True Empty State: No data exists at all */
...
//                     Click "New Assessment" to create one
//                   </p>
//                 </div>
//               ) : filteredList.length === 0 ? (
//                 /* Filtered Empty State: Data exists, but not for this tab */
//                 <div className="text-center py-12">
...
//                           </span>
//                           <span>
//                             {q.totalQuestions ?? q.questionIds?.length ?? 0}{" "}
//                             questions
//                           </span>
//                         </div>
//                       </div>
...
//                           </button>
//                         )}
//                         {/* Optional: Add Edit button here for drafts if needed */}
//                       </div>
//                     </motion.div>
//                   );
//                 })
...
//                         ${allSelected ? "bg-indigo-600 text-white border-indigo-600" : "bg-white text-indigo-600 border-indigo-300 hover:bg-indigo-50"}`}
//                     >
//                       <div
...
//                                         )}
//                                       </div>
//                                       <span className="text-slate-400 w-5 flex-shrink-0 font-medium">
//                                         {qi + 1}.
//                                       </span>
//                                       <p className="text-slate-700 flex-1 leading-relaxed text-xs">
//                                         {q.text}
//                                       </p>
//                                     </div>
//                                   );
...
//                         {
//                           label: "Questions",
//                           value: `${selectedQIds.length} selected`,
//                         },
//                       ].map(({ label, value }) => (
//                         <div key={label}>
...
// //   FileText, Shield, RefreshCw, Save, Clock
// // } from "lucide-react";
// // import tprmService from "../services/tprmService";
...
// //       const [qs, allUsers, qlist] = await Promise.all([
// //         tprmService.getQuestionnaires(organization),
// //         fetch(`http://localhost:4000/api/users?organization=${organization}`, {
// //           headers: {
// //             "Authorization": `Bearer ${sessionStorage.getItem("token")}`,
// //             "Content-Type": "application/json"
// //           }
...
// //       setQuestionnaires(Array.isArray(qs) ? qs : []);
// //       const tprmUsers = Array.isArray(allUsers)
// //         ? allUsers.filter(u => Array.isArray(u.modules) && u.modules.includes("tprm"))
// //         : [];
// //       setVendors(tprmUsers);
// //       setQuestions(Array.isArray(qlist) ? qlist : []);
// //     } finally {
...
// //     let list = [...questionnaires];
// //     if (statusFilter !== "All") {
// //       if (statusFilter === "Due") list = list.filter(q => ["SENT", "SUBMITTED", "RESUBMITTED"].includes(q.status));
// //       else if (statusFilter === "Received") list = list.filter(q => q.status === "UNDER_REVIEW");
// //       else if (statusFilter === "Completed") list = list.filter(q => ["APPROVED", "REJECTED"].includes(q.status));
// //       else list = list.filter(q => q.status === statusFilter);
// //     }
// //     // Sorting by most recent date at top
// //     return list.sort((a, b) => new Date(b.dueDate) - new Date(a.dueDate));
// //   }, [questionnaires, statusFilter]);
...
// //   const toggleQuestion = (id) => setSelectedQIds(prev => prev.includes(id) ? prev.filter(x => x !== id) : [...prev, id]);

// //   const toggleTopic = (key, qs) => {
// //     const ids = qs.map(q => q.id);
// //     const allSel = ids.every(id => selectedQIds.includes(id));
// //     setSelectedQIds(prev => allSel ? prev.filter(id => !ids.includes(id)) : [...new Set([...prev, ...ids])]);
// //   };
...
// //       if (editingId) {
// //         await tprmService.updateQuestionnaire(editingId, payload);
// //       } else {
// //         const created = await tprmService.createQuestionnaire({
...
// //   return (
// //     <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
// //       <motion.div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col" initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}>

// //         {/* Header - EXACT SAME AS OLD CODE */}
// //         <div className="flex items-center justify-between p-6 border-b border-slate-100">
// //           <div className="flex items-center gap-3">
...
// //               <p className="text-xs text-slate-500">
// //                 {view === "list" ? `${questionnaires.length} assessments` : "Create new assessment"}
// //               </p>
// //             </div>
// //           </div>
// //           <div className="flex items-center gap-2">
// //             {view === "list" && (
// //               <button onClick={() => { setView("create"); setStep(1); }} className="px-3 py-2 bg-indigo-600 text-white text-xs font-semibold rounded-xl hover:bg-indigo-700 shadow flex items-center gap-1">
// //                 <Plus size={13} /> New Assessment
// //               </button>
// //             )}
// //             {view !== "list" && (
// //               <button onClick={() => setView("list")} className="px-3 py-2 bg-slate-100 text-slate-600 text-xs font-medium rounded-xl hover:bg-slate-200">
// //                 ← Back to List
// //               </button>
// //             )}
...
// //           {view === "list" ? (
// //             <div className="p-6 space-y-3">
// //               {/* Sorting Tabs from VendorReportModal.js */}
// //               <div className="flex gap-1 flex-wrap mb-4">
// //                 {["All", "Due", "Received", "Completed"].map((tab) => (
// //                   <button
// //                     key={tab}
// //                     onClick={() => setStatusFilter(tab)}
// //                     className={`px-4 py-1.5 rounded-lg text-xs font-medium transition-all ${
// //                       statusFilter === tab ? "bg-indigo-600 text-white shadow-sm" : "bg-slate-100 text-slate-600 hover:bg-slate-200"
// //                     }`}
// //                   >
// //                     {tab}
// //                   </button>
...
// //               {loading ? (
// //                 <div className="flex items-center justify-center py-10"><RefreshCw className="animate-spin text-slate-400" /></div>
// //               ) : filteredList.map((q) => {
// //                 const cfg = STATUS_CONFIG[q.status] || STATUS_CONFIG.DRAFT;
// //                 return (
// //                   <div key={q.id} className="flex items-center gap-4 p-4 border border-slate-100 rounded-xl hover:bg-slate-50 transition-colors">
...
// //                       <p className="text-xs text-slate-500">Vendor: {q.vendorName} | Due: {q.dueDate} | {q.questionIds?.length || 0} questions</p>
// //                     </div>
...
// //           ) : (
// //             <div className="p-6">
// //               {/* Steps - EXACT SAME AS OLD CODE */}
// //               <div className="flex gap-2 mb-6">
// //                 <button onClick={() => setStep(1)} className={`px-4 py-1.5 rounded-lg text-xs font-medium ${step === 1 ? "bg-indigo-600 text-white" : "bg-slate-100 text-slate-500"}`}>1. Assessment Details</button>
// //                 <button onClick={() => setStep(2)} className={`px-4 py-1.5 rounded-lg text-xs font-medium ${step === 2 ? "bg-indigo-600 text-white" : "bg-slate-100 text-slate-500"}`}>2. Select Questions</button>
// //               </div>

// //               {step === 1 ? (
// //                 <div className="space-y-4 max-w-lg">
// //                   <div>
// //                     <label className="text-xs font-medium text-slate-600 block mb-1">Assessment Title *</label>
// //                     <input type="text" value={form.title} onChange={e => setForm({...form, title: e.target.value})} className="w-full border rounded-xl px-3 py-2 text-sm" placeholder="Title..." />
// //                   </div>
// //                   <div>
// //                     <label className="text-xs font-medium text-slate-600 block mb-1">Assign Vendor *</label>
// //                     <select value={form.vendorId} onChange={handleVendorChange} className="w-full border rounded-xl px-3 py-2 text-sm bg-white">
// //                       <option value="">Select vendor...</option>
// //                       {vendors.map(v => <option key={v._id || v.id} value={v._id || v.id}>{v.name || v.username}</option>)}
// //                     </select>
// //                   </div>
// //                   <div>
// //                     <label className="text-xs font-medium text-slate-600 block mb-1">Due Date *</label>
// //                     <input type="date" value={form.dueDate} onChange={e => setForm({...form, dueDate: e.target.value})} className="w-full border rounded-xl px-3 py-2 text-sm" />
// //                   </div>
// //                   {error && <p className="text-sm text-red-500">{error}</p>}
// //                   <button
// //                     onClick={() => {
// //                       if (!form.title || !form.vendorId || !form.dueDate) return setError("Missing fields");
// //                       setError(""); setStep(2);
// //                     }}
// //                     className="px-6 py-2.5 bg-gradient-to-r from-indigo-500 to-indigo-600 text-white text-sm font-semibold rounded-xl shadow hover:shadow-md transition-all"
// //                   >
...
// //                       <input type="text" placeholder="Search questions..." value={search} onChange={e => setSearch(e.target.value)} className="w-full pl-10 pr-4 py-2 border rounded-xl text-sm" />
// //                     </div>
// //                     <button onClick={toggleSelectAll} className="px-3 py-2 border rounded-xl text-xs">{allSelected ? "Deselect All" : "Select All"}</button>
// //                   </div>

// //                   {/* Question Selection List - EXACT SAME AS OLD CODE */}
// //                   <div className="flex-1 overflow-y-auto space-y-3">
// //                     {Object.entries(grouped).map(([key, qs]) => {
// //                       const [tNo, tName] = key.split("||");
// //                       const isExpanded = expandedTopics[key];
// //                       return (
// //                         <div key={key} className="border rounded-xl overflow-hidden">
// //                           <div className="flex items-center justify-between p-3 bg-slate-50">
// //                             <div className="flex items-center gap-3">
// //                               <button onClick={() => toggleTopic(key, qs)} className={`w-5 h-5 rounded border flex items-center justify-center ${qs.every(q => selectedQIds.includes(q.id)) ? "bg-indigo-600 border-indigo-600" : "bg-white border-slate-300"}`}>
// //                                 {qs.every(q => selectedQIds.includes(q.id)) && <Check size={12} className="text-white" />}
// //                               </button>
// //                               <h4 className="text-sm font-bold text-slate-700">{tNo}. {tName}</h4>
// //                             </div>
// //                             <button onClick={() => setExpandedTopics(prev => ({...prev, [key]: !prev[key]}))}><ChevronDown size={16}/></button>
// //                           </div>
// //                           {isExpanded && (
// //                             <div className="p-2 space-y-1">
// //                               {qs.map(q => (
// //                                 <div key={q.id} onClick={() => toggleQuestion(q.id)} className="flex items-start gap-3 p-2 hover:bg-slate-50 cursor-pointer">
// //                                   <div className={`mt-0.5 w-4 h-4 rounded border flex-shrink-0 flex items-center justify-center ${selectedQIds.includes(q.id) ? "bg-indigo-600 border-indigo-600" : "bg-white border-slate-300"}`}>
// //                                     {selectedQIds.includes(q.id) && <Check size={10} className="text-white" />}
// //                                   </div>
...
// //                   {/* Submit Button Section - UI IS SAME, ONLY TEXT CHANGED */}
// //                   <div className="pt-6 border-t mt-4 flex items-center justify-between">
// //                     <span className="text-xs text-slate-500">{selectedQIds.length} selected</span>
// //                     <div className="flex gap-3">
// //                       <button onClick={() => setStep(1)} className="px-6 py-2.5 bg-slate-100 text-slate-600 text-sm font-semibold rounded-xl">← Back</button>
// //                       <button onClick={handleSubmit} disabled={saving || selectedQIds.length === 0} className="px-6 py-2.5 bg-indigo-600 text-white text-sm font-bold rounded-xl flex items-center gap-2">
